...
const pinSequenceLink: DbLinker<Pin['channel'], Pin, BaseSequence['id'], BaseSequence> = (db) => ({
    ...
    onUpdate: (sequence, { channel }, oldChannel) => {
        ...
        return {
            ...sequence,
            orders: sequence.orders.map(o => o.channel === oldChannel ? { ...o, channel, } : o)
        }
    },
...
const sequenceEventLink: DbLinker<BaseSequence['id'], BaseSequence, BaseSequenceEvent['id'], BaseSequenceEvent> = (db) => ({
    db,
    predict: ({ foreignItem, key, oldKey }) => foreignItem.sequenceId === (oldKey || key),
    onDelete: "CASCADE",
    onUpdate: (event, sequence, oldKey) => oldKey ? ({ ...event, sequenceId: sequence.id }) : event
})

const cronCSLink: DbLinker<BaseCron['id'], BaseCron, void, CronSequence> = (db) => ({
    db,
