...
interface CronSequenceLinkInterface {
    linkSequence: (sequenceId: BaseSequence['id'], cronsIds: BaseCron['id'][]) => BaseSequence | undefined
    linkCron: (cronId: BaseCron['id'], sequencesIds: BaseSequence['id'][]) => BaseCron | undefined
...
class CronSequenceLink implements CronSequenceLinkInterface {
    ...
    linkSequence = (sequenceId: BaseSequence['id'], cronsIds: BaseCron['id'][]) => {
        const exists = this.sequenceDb.exists(sequenceId)
        if (!exists) return

        const existingCron = cronsIds.map(this.cronDb.findByKey).filter(c => c) as BaseCron[]
    ...
    linkCron = (cronId: BaseCron['id'], sequencesIds: BaseSequence['id'][]) => {
        ...
        const existingSequences = sequencesIds.map(this.sequenceDb.findByKey).filter(c => c) as BaseSequence[]
