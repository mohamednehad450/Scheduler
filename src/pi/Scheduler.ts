...
class Scheduler extends EventEmitter implements SchedulerInterface<BaseSequence['id']> {
    ...
    start = async () => {
        ...
        const eventHandler = (event: SequenceEvent['eventType']) => (id: BaseSequence['id']) => {
        ...
        const channelChange: PinManagerEvents['channelChange'] = (change) => this.emit('channelChange', change)
