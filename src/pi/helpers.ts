...
type RunError = "ALREADY_RUNNING" | "CHANNEL_IN_USE" | "CHANNEL_NOT_INITIALIZED"
...
export type PinManagerEvents = {
    ...
    runError: (error: RunError, sequenceId: BaseSequence['id'], badChannel?: Pin['channel'][]) => void
};


export const orderToTimer = (pin: Pin, order: BaseSequence['orders'][number], onChange: (state: { [key: Pin['channel']]: boolean }) => void) => ({
    pin,
...
export const clearSequenceTimer = async (sequenceTimer: SequenceTimer) => {
    ...
    sequenceTimer.orderTimers.forEach(({ startTimer, stopTimer: closeTimer, pin, }) => {
        clearTimeout(startTimer)
        clearTimeout(closeTimer)
        pins.add(pin)
    })
    clearTimeout(sequenceTimer.cleanup);
    return await Promise.all(Array.from(pins).map(async (pin) => {
        await pinOff(pin)
        return pin
    }))
}
